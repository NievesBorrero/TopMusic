...
public class TopMusic{
	 ...
	 * @param cancion
	 */
	public void add(String titulo, String artista, String fecha) {
		Cancion cancion=new Cancion(titulo, artista, fecha);
		getTop().add(cancion);
	}
	 ...
	 * @param index
	 * @return true o false, en función de si ha sido posible eliminar el elemento del index dado por parámetro o no.
	 */
	 ...
	 * @param index
	 * @return true si modifica la posicion o false, si no puede porque el índice introducido como parámetro no es válido.
	 */
	boolean up(int index) {		
		...
		// Añade en una posición anterior, la canción con el índice introducido borrando esa canción del índice en el que estaba.
		return true;
	}
	...
	boolean pullOut(int index) {
		if (!comprobarIndex(index - 1) || top.size() < 10)
			return false;
		top.add(10, getTop().remove(index - 1)); // Añade en una posición siguiente al top10, borrando la canción de su lugar.
		return true;
	}
